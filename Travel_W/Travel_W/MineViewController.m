...
- (IBAction)RegisteredAction:(UIButton *)sender forEvent:(UIEvent *)event;
...
- (IBAction)RegisteredAction:(UIButton *)sender forEvent:(UIEvent *)event {
    RegisteredViewController *registered = [self.storyboard instantiateViewControllerWithIdentifier:@"Registered"];

    //初始化导航控制器
    UINavigationController *nc = [[UINavigationController alloc]initWithRootViewController:registered];
    //动画效果
    //nc.modalTransitionStyle = UIModalTransitionStyleFlipHorizontal;
    //导航条隐藏掉
    nc.navigationBarHidden = YES;
